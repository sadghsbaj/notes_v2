...
```
<form>  <ion-list>    <ion-item>      <ion-label>E-Mail Address</ion-label>      <ion-input appAutofill type="email" name="email" autocomplete="email" [(ngModel)]="email" required email></ion-input>    </ion-item>    <ion-item>      <ion-label>Password</ion-label>      <ion-input appAutofill type="password" name="password" autocomplete="current-password" required [(ngModel)]="password"></ion-input>    </ion-item>  </ion-list>  <ion-button type="submit">Submit</ion-button></form>
```
...
```
<form>  <ion-list>    <ion-item>      <ion-label>E-Mail Address</ion-label>      <ion-input type="email" name="email" autocomplete="email" required email></ion-input>    </ion-item>    <ion-item>      <ion-label>Password</ion-label>      <ion-input id="pwd" type="password" name="password" autocomplete="current-password" required></ion-input>    </ion-item>  </ion-list>  <ion-button type="submit">Submit</ion-button></form>
```
...
```
    document.getElementById('pwd').children[0].addEventListener('change', (e) => {      this.password = (e.target as any).value;          });
```
...
## Set Capacitor Server Hostname[​](#set-capacitor-server-hostname "Direct link to Set Capacitor Server Hostname")
...
## Configuration for iOS[​](#configuration-for-ios "Direct link to Configuration for iOS")

### Configuration in XCode[​](#configuration-in-xcode "Direct link to Configuration in XCode")

Open your project in XCode and navigate to `Signing & Capabilities`.

![XCode capabilities](/docs/assets/images/xcode-capabilities-cb2c8c38b3816a1ecd804fd190e7e090.png)
...
### Apple App Site Association File[​](#apple-app-site-association-file "Direct link to Apple App Site Association File")
...
![Diagnostics](/docs/assets/images/diagnostics-f0c6a4230e3fa72a3e9519b47267fb7b.png)

A green checkmark indicates a validation configuration whilst a yellow alert indicates a problem.

#### Other Validation Tools[​](#other-validation-tools "Direct link to Other Validation Tools")
...
![Save Credentials](/docs/assets/images/save-password-bebf53bd9b1d3f55f2cdc80b0cd36849.png)

### Autofill in Action[​](#autofill-in-action "Direct link to Autofill in Action")
...
![Autofill Credentials](/docs/assets/images/auto-fill-28069a501087900fdf567432a3de2075.png)

## Configuration for Android[​](#configuration-for-android "Direct link to Configuration for Android")

Follow the [Android Deep Links Guide](/docs/guides/deep-links#android-configuration) to create a Site Association File and associated `AndroidManifest.xml` changes and additionally verify:
...
## Configuration for Web[​](#configuration-for-web "Direct link to Configuration for Web")

Follow the [Deep Links Guide](/docs/guides/deep-links#website-configuration) if you are targeting the web.
...
![iOS Safari](/docs/assets/images/ios-safari-cc8354620d00d328f91b7b7ff5a5c4e2.png)

## iOS Troubleshooting[​](#ios-troubleshooting "Direct link to iOS Troubleshooting")
...
#### The autofill option for passwords does not appear. What should I check?[​](#the-autofill-option-for-passwords-does-not-appear-what-should-i-check "Direct link to The autofill option for passwords does not appear. What should I check?")
...
#### Do I need the `AutoFill Credential Provider` capability?[​](#do-i-need-the-autofill-credential-provider-capability "Direct link to do-i-need-the-autofill-credential-provider-capability")

No, this capability is not required.

#### Do I need the `webcredentials:domain` in Associated Domains?[​](#do-i-need-the-webcredentialsdomain-in-associated-domains "Direct link to do-i-need-the-webcredentialsdomain-in-associated-domains")

No, you only need the `applinks:domain` in the Associated Domains.

#### Do I need the `webcredentials` in `apple-app-site-association`?[​](#do-i-need-the-webcredentials-in-apple-app-site-association "Direct link to do-i-need-the-webcredentials-in-apple-app-site-association")

No, you only need the `applinks` and `appID` properties.

#### The Apple Validation Tool reports `Error cannot parse app site association`[​](#the-apple-validation-tool-reports-error-cannot-parse-app-site-association "Direct link to the-apple-validation-tool-reports-error-cannot-parse-app-site-association")
...
#### Apples Documentation is different from these Instructions[​](#apples-documentation-is-different-from-these-instructions "Direct link to Apples Documentation is different from these Instructions")
