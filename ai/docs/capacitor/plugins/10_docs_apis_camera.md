...
### getPhoto(...)[​](#getphoto "Direct link to getPhoto(...)")
...
### pickImages(...)[​](#pickimages "Direct link to pickImages(...)")
...
### pickLimitedLibraryPhotos()[​](#picklimitedlibraryphotos "Direct link to pickLimitedLibraryPhotos()")
...
### getLimitedLibraryPhotos()[​](#getlimitedlibraryphotos "Direct link to getLimitedLibraryPhotos()")
...
#### Photo[​](#photo "Direct link to Photo")

| Prop | Type | Description | Since |
| --- | --- | --- | --- |
| **`base64String`** | `string` | The base64 encoded string representation of the image, if using [CameraResultType.Base64](#cameraresulttype). | 1.0.0 |
| **`dataUrl`** | `string` | The url starting with 'data:image/jpeg;base64,' and the base64 encoded string representation of the image, if using [CameraResultType.DataUrl](#cameraresulttype). Note: On web, the file format could change depending on the browser. | 1.0.0 |
| **`path`** | `string` | If using [CameraResultType.Uri](#cameraresulttype), the path will contain a full, platform-specific file URL that can be read later using the Filesystem API. | 1.0.0 |
...
#### ImageOptions[​](#imageoptions "Direct link to ImageOptions")
...
| **`allowEditing`** | `boolean` | Whether to allow the user to crop or make small edits (platform specific). On iOS it's only supported for [CameraSource.Camera](#camerasource), but not for [CameraSource.Photos](#camerasource). |  | 1.0.0 |
| **`resultType`** |
```
CameraResultType
```
...
#### GalleryPhotos[​](#galleryphotos "Direct link to GalleryPhotos")

| Prop | Type | Description | Since |
| --- | --- | --- | --- |
| **`photos`** | `GalleryPhoto[]` | Array of all the picked photos. | 1.2.0 |

#### GalleryPhoto[​](#galleryphoto "Direct link to GalleryPhoto")
...
#### GalleryImageOptions[​](#galleryimageoptions "Direct link to GalleryImageOptions")
...
#### CameraPluginPermissions[​](#camerapluginpermissions "Direct link to CameraPluginPermissions")
...
#### CameraPermissionState[​](#camerapermissionstate "Direct link to CameraPermissionState")
...
#### CameraPermissionType[​](#camerapermissiontype "Direct link to CameraPermissionType")

`'camera' | 'photos'`

### Enums[​](#enums "Direct link to Enums")

#### CameraResultType[​](#cameraresulttype "Direct link to CameraResultType")
...
#### CameraSource[​](#camerasource "Direct link to CameraSource")
...
#### CameraDirection[​](#cameradirection "Direct link to CameraDirection")
