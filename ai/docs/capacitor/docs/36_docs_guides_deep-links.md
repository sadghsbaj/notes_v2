...
Here's what it looks like in practice. In this example, the user has the native app installed. They tap on app links from an email and are brought directly into the app itself. First, the root link is tapped ([https://beerswift.app](https://beerswift.app)), which directs the user to the main app page. Next, a deep link is tapped ([https://beerswift.app/tabs/tab3](https://beerswift.app/tabs/tab3)) bringing the user to the Tab3 page.
...
For illustrative purposes, [https://beerswift.app](https://beerswift.app) will be used as the web app link.

## Deep Link Routing using the Capacitor App API[​](#deep-link-routing-using-the-capacitor-app-api "Direct link to Deep Link Routing using the Capacitor App API")
...
```
initializeApp() {    App.addListener('appUrlOpen', (event: URLOpenListenerEvent) => {        this.zone.run(() => {            // Example url: https://beerswift.app/tabs/tab2            // slug = /tabs/tab2            const slug = event.url.split(".app").pop();            if (slug) {                this.router.navigateByUrl(slug);            }            // If no match, do nothing - let regular routing            // logic take over        });    });}
```
...
```
const AppUrlListener: React.FC<any> = () => {  let history = useHistory();  useEffect(() => {    App.addListener('appUrlOpen', (event: URLOpenListenerEvent) => {      // Example url: https://beerswift.app/tabs/tab2      // slug = /tabs/tab2      const slug = event.url.split('.app').pop();      if (slug) {        history.push(slug);      }      // If no match, do nothing - let regular routing      // logic take over    });  }, []);  return null;};export default AppUrlListener;
```
...
```
const App: React.FC = () => {  return (    <IonApp>      <IonReactRouter>        <AppUrlListener></AppUrlListener>        <IonRouterOutlet>          <Route path="/home" component={Home} exact={true} />          <Route exact path="/" render={() => <Redirect to="/home" />} />        </IonRouterOutlet>      </IonReactRouter>    </IonApp>  );};
```
...
```
const VueApp = new Vue({  router,}).$mount('#app');
```
...
## Creating Site Association Files[​](#creating-site-association-files "Direct link to Creating Site Association Files")

In order for Apple and Google to permit deep links to open your app, a two-way association between your website and app must be created. One file for each must be created and placed within a `.well-known` folder on your website, like so: [https://beerswift.app/.well-known/](https://beerswift.app/.well-known/).
...
### Create Site Association File[​](#create-site-association-file "Direct link to Create Site Association File")
...
![iOS Identifier Config](/docs/assets/images/ios-config-03bffaecf4534e7c226c69fb76a58184.png)
...
Next, upload the file to your web site (hosted on HTTPS), then validate that it's configured correctly using Apple's tool [here](https://search.developer.apple.com/appsearch-validation-tool/). The URL should follow this format: [https://beerswift.app/.well-known/apple-app-site-association](https://beerswift.app/.well-known/apple-app-site-association)

### Add Associated Domain[​](#add-associated-domain "Direct link to Add Associated Domain")
...
![Xcode Associated Domain](/docs/assets/images/xcode-associated-domain-31750426e233123198b9d6edd347df84.png)
...
### Create Site Association File[​](#create-site-association-file-1 "Direct link to Create Site Association File")
...
![Keytool output](/docs/assets/images/keystore-sha256-8d06695d59fbbf891f5c04c404f5e6e2.png)
...
![Android Identifier Config](/docs/assets/images/android-config-032c0bc4890ee44d1c7e788cd6470390.png)
...
### Add Intent Filter[​](#add-intent-filter "Direct link to Add Intent Filter")
...
```
<intent-filter android:autoVerify="true">    <action android:name="android.intent.action.VIEW" />    <category android:name="android.intent.category.DEFAULT" />    <category android:name="android.intent.category.BROWSABLE" />    <data android:scheme="https" android:host="beerswift.app" /></intent-filter>
```

The complete Activity should look similar to this:

```
<activity    android:configChanges="orientation|keyboardHidden|keyboard|screenSize|locale"    android:name="com.netkosoft.beerswift.MainActivity"    android:label="@string/title_activity_main"    android:theme="@style/AppTheme.NoActionBarLaunch"    android:launchMode="singleTask">    <intent-filter>        <action android:name="android.intent.action.MAIN" />        <category android:name="android.intent.category.LAUNCHER" />    </intent-filter>    <intent-filter android:autoVerify="true">        <action android:name="android.intent.action.VIEW" />        <category android:name="android.intent.category.DEFAULT" />        <category android:name="android.intent.category.BROWSABLE" />        <data android:scheme="https" android:host="beerswift.app" />    </intent-filter></activity>
```

## Website Configuration[​](#website-configuration "Direct link to Website Configuration")
...
### NuxtJS[​](#nuxtjs "Direct link to NuxtJS")
