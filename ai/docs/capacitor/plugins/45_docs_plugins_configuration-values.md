...
## Defining Configuration Values[​](#defining-configuration-values "Direct link to Defining Configuration Values")
...
```
{  appId: 'com.company.app',  ...  plugins: {    MyCoolPlugin: {      style: "dark",      iconColor: '#FF0000'    }  }}
```
...
```
/// <reference types="@capacitor/cli" />declare module '@capacitor/cli' {  export interface PluginsConfig {    MyCoolPlugin?: {      /**       * Override the cool theme style if your app doesn't support light/dark theme changes.       *       * @since 1.0.0       * @example "light"       */      style?: 'dark' | 'light';      /**       * Color of the cool icon in hex format, #RRGGBB or #RRGGBBAA.       *       * @since 1.0.0       * @default #ffffff       * @example "#FF9900"       */      iconColor?: string;    };  }}
```
...
```
/// <reference types="@capacitor-community/my-cool-plugin" />import { CapacitorConfig } from '@capacitor/cli';const config: CapacitorConfig = {  appId: "com.company.app",  ...  plugins: {    MyCoolPlugin: {      style: "dark",      iconColor: "#034821"    }  }}export default config;
```

## Accessing Configuration Values[​](#accessing-configuration-values "Direct link to Accessing Configuration Values")
...
```
if let style = getConfig().getString("style") {  // Set the style}
```

For Android:

```
String style = getConfig().getString("style");if(style) {  // Set the style}
```
